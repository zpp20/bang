...
def get_result(attractor_cum_index :list[npt.NDArray[np.uint32]]):
    ...
    while True:
        ...
        if not increment_states(current, max_lens):
            return [np.uint32(0)] + cum_result, result_size, list(accumulate(max_lens, add)), reduce(mul, max_lens)
...
def corss_attractors_gpu(
    ...
    attractors_global = cuda.to_device(np.concatenate(attractor_list, dtype=np.uint32), stream=stream)
    attractors_index = cuda.to_device(np.array(list(accumulate(attractor_cum_index, 
                                                               lambda x,y : np.concatenate((x,np.array(list(map(lambda z : z+ x[-1], y[1:])), dtype = np.uint32)))
                                                               ))[-1], dtype=np.uint32), stream=stream)
    blocks_sizes = cuda.to_device(np.array([attractor.shape[0] for attractor in attractor_list], dtype=np.uint32), stream=stream)
