...
def corss_attractors_gpu(attractor_list :list[npt.NDArray[np.int32]], nodes :list[list[int]], attractor_start :list[int], blocks_indices :list[int]):
    ...
    attractors_index = cuda.to_device(np.array(attractor_start))
...
def cross_attractors(
    attractors_global, #list of 2d arrays
    block_indices, 
    ...
    tmp = comb_id
    indices = np.zeros(block_indices.shape)
    lengths = indices.copy()
    current_states = np.zeros((indices.size, 3))
    
    for i in range(block_indices.size):
        indices[i] = attractors_cum_index[i] + (tmp % block_indices[i])
        tmp /= block_indices[i]
    for i in range(indices.size):
        lengths[i] = attractors_global[indices[i]][0][0]
    ...
    while True:
        ...
        for i in range(len(select_order)):
            ...
            read = (1 << current_states[select_order[i]][2]) & (attractors_global[indices[i]][current_states[select_order[i]][0]][current_states[select_order[i]][1]]) >> current_states[select_order[i]][2]
        ...
        result[result_start : result_end] = new_node
